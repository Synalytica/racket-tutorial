...
First, we need a helper function, viz, =parse-assumptions= for the special =assume= case.
...
Next, we can define the main =parse= function
...
(define (parse exp)
    (cond
      ...
      [(and (list? exp) (= (length exp) 2) (eq? (first exp) '!)) (unaryop 'neg (parse (second exp)))]  ;; unary operator
...
** Lookup (You need to implement this)
  ...
  First, we need a few helpers for the symbol defintion checking, namely,
  =is-in= (checks if element is in list) and =get-index= (gets index, if in list).

#+NAME: lookup-env-helpers
#+BEGIN_SRC racket
...
(define lookup-env
    (λ (e sym)
      (cases env e
        ...
        (extended-env (symbols values e)
                      (if (is-in sym symbols)
                          (list-ref values (get-index sym symbols))
                          (lookup-env e sym)
                          )
                      )
        )
...
(define eval-ast
    (λ (tree environment)
      (cases ast tree
        [num (n) n]
        [bool (b) b]
        [id-ref (exp) (let ((mock-env environment)) (lookup-env mock-env 'x))]
        [unaryop (operand rand1) (cond [(equal? (op-interpretation operand) 'neg)
                                        (not (typecheck-num (eval-ast rand1 environment)))])]
        [binop (operand lhs rhs) (cond [(equal? (op-interpretation operand) /)
                                        (/ (typecheck-num (eval-ast lhs environment))
                                           (check-non-zero (typecheck-num (eval-ast rhs environment)))
                                           )]
                                       [else ((op-interpretation operand)
                                              (typecheck-num (eval-ast lhs environment))
                                              (typecheck-num (eval-ast rhs environment))
                                              )])]
